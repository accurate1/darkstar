...
"c:\program files\MariaDB 10.2\bin\mysqladmin"-h localhost -u root -pBuccaneers1 DROP dspdb

ECHO Creating Database dspdb
"c:\program files\MariaDB 10.2\bin\mysqladmin"-h localhost -u root -pBuccaneers1 CREATE dspdb

ECHO Loading dspdb tables into the database
cd c:\darkstar\sql
FOR %%X IN (*.sql) DO ECHO Importing %%X & "c:\program files\MariaDB 10.2\bin\mysql" dspdb -h localhost -u root -pBuccaneers1 < %%X
